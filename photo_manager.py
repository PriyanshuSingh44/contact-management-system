...
In Docker, the PHOTOS_DIR env var overrides the default path so
photos are written into the named volume (/app/photos).
"""
...
class PhotoManager:
    ...
    def __init__(self, photos_dir: str = "photos") -> None:
        ...
        photos_dir : str
            ...
            Ignored when the PHOTOS_DIR environment variable is set (Docker).
        """
        if os.environ.get("PHOTOS_DIR"):
            # Docker: use the explicit path from the environment variable
            self._photos_dir: Path = Path(os.environ["PHOTOS_DIR"])
        else:
            self._photos_dir = Path(__file__).parent.resolve() / photos_dir
